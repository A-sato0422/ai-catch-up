...
- フェーズ: **フェーズ A〜H ほぼ全完了（2026-07-10）**。残りはユーザー作業のみ
...
- [x] 難易度フィルタ画面が切替期に疎になることの表示確認（空状態が出ること）（2026-07-10・Playwright。Supabaseレスポンスを空配列にモックして確定的に検証。「このカテゴリは今日は静からしいよ」「新しい記事が来たらすぐここに並ぶよ」の表示を確認）
