...
- [x] ホーム画面
  ...
  - [x] テキスト「krobo」は現在白色だが、目や口と同じ水色ベースの色にして目立つデザインに修正
    ...
    - [x] （2026-07-16 追加指示・最終スタイル）「水色系にしつつ浮かないように」との指示で `rgba(84, 215, 246, 0.75)`（目・口と同じ #54d7f6 の 75% 透過）・影なし・ブレンドなしに決定。半透明で胴体の陰影が透け、プリント風に馴染む。SVG 埋め込み側とフォールバック span の両方に適用し、SP/PC 拡大スクリーンショットで確認済み
