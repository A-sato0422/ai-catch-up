...
内容ベースで重要度を採点。破壊的変更/メジャーアップデート/広く影響する一次情報ほど高く、ニッチな小ネタは低く。**指標（Qiita ストック等）が無いソースも横断評価できる**よう、LLM 判定を主とする。あわせて `importance_reason`（例「破壊的変更」）を 10〜15 字で出力し、カードでスコアと併記する。
...
### 6.4 「今日の重要 TOP5」選定
**直近 24 時間のローリングウィンドウ**（暦日境界ではなく現在時刻からの相対時間。D-023）で公開された記事を `importance_score` 降順で上位 5 件。**選定対象はユーザーがチェック中（ホームに表示中）のカテゴリの記事のみ**に絞る（フロント側フィルタ。自由枠を1つも選択していない場合はフィルタをスキップし全件を対象にする）。同点時のタイブレークは、(1) Qiita はストック数、(2) GitHub Releases は新しさ、(3) それ以外は `published_at` の新しさ、の順で補助的に使う。
...
| 区分 | 画面 | 内容 | 並び順 |
|---|---|---|---|
| 固定 | 今日の重要 TOP5 | 直近 24 時間・チェック中カテゴリに絞った重要度上位 5 件 | importance_score 降順 |
