...
- フェーズ: **フェーズ 5（クリーンアップ）完了 / フェーズ 6 残タスク（Supabase 実データ接続）着手前**
- 次にやること: フェーズ 6 残タスク（Supabase 実データ接続フック・空状態表示・Vercel デプロイ）
- 直近の決定: フェーズ 4+5 完了（2026-06-20）: collect.ts（fail-soft・BACKFILL_DAYS・バッチ内重複排除）/ cleanup.ts / GitHub Actions 2ワークフロー実装。全テスト（計 63 件）パス。root vitest を src/ 限定に修正（フロント 0 件・batch 63 件で分離）。
...
- [x] `collect.ts`: fetch → URL正規化 → バッチ内重複排除 → DB重複排除 → enrich → upsert（2026-06-20）
- [x] 初回バックフィル対策（`BACKFILL_DAYS` env で取得期間を絞る）（2026-06-20）
- [x] fail-soft（ソース単位 & 記事単位の例外処理とログ）（2026-06-20）
- [x] ローカル実行確認（`npm run collect`）← Supabase 接続・ソース取得・fail-soft 確認済み（2026-06-20）。Gemini Flash quota が 0 でenrich 全件失敗のため、Flash-Lite フォールバックのバグを修正（429 で即切替）。Flash-Lite での enrich は quota リセット後に確認予定
- [x] `.github/workflows/collect.yml`（cron 07:00 JST + workflow_dispatch）（2026-06-20）

## フェーズ 5: クリーンアップ

- [x] `cleanup.ts`: 非お気に入り & `RETENTION_DAYS` 日超を物理削除（2026-06-20）
- [x] `.github/workflows/cleanup.yml`（週次 cron 日曜 08:00 JST）（2026-06-20）
...
- [x] Supabase 読み取りフック（実データ接続）— `src/hooks/useArticles.ts` 新規作成、スクリーン別クエリ実装（2026-06-20）
- [x] 空状態の表示（データなし時）— loading / error / empty 各状態を ListPage に実装（2026-06-20）
- [×] Vercel デプロイ
