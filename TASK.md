...
- フェーズ: **フェーズ 0（Supabase セットアップ待ち）/ フェーズ 6（フロントエンド MVP）完了 / バッチ 未着手**
- 次にやること: Supabase プロジェクト作成（手動）→ SQL 実行 → API キー取得 → フェーズ 1（バッチ共通基盤）
- 直近の決定: フロントは React + TypeScript + Vite + Tailwind CSS + React Router v6 でルート直下に実装。ダークモード対応済み。モックデータで動作確認済み。migration SQL・.env.example・batch/src/ 骨格・型定義を 2026-06-20 に作成。
