...
- フェーズ: **フェーズ 1（共通基盤）完了 / フェーズ 2（ソースアダプタ）着手前**
- 次にやること: フェーズ 2（ソースアダプタ実装）— githubReleases から順に実装
- 直近の決定: フロントは React + TypeScript + Vite + Tailwind CSS + React Router v6 でルート直下に実装。ダークモード対応済み。フェーズ 1 完了（2026-06-20）: normalizeUrl / Supabase クライアント / upsert ロジック実装済み。全テスト（計 12 件）パス。
