...
重要度の基準（カテゴリを問わず「読者の行動・判断をどれだけ変えるか」で採点する。update 系だから高い/tips 系だから低いという判断はしない）:
- 9-10: 必読。破壊的変更・メジャーリリース、業界を揺るがす重大ニュース、多くの読者の使い方を変える決定版ノウハウ・導入事例
- 7-8: 重要。主要な新機能、広く役立つ実践的 Tips、著名企業の本格導入事例、大きな業界動向
- 4-6: 通常。小規模なアップデート、特定用途で役立つ Tips・事例、業界の定常的な動き
- 1-3: ニッチな小ネタ・ごく限定的な話題
